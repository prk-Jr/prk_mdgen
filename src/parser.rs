...
fn parse_file_code(content: &str) -> Vec<ParsedFile> {
    let mut results = Vec::new();
    lazy_static! {
        static ref FILE_TAG_REGEX: Regex = Regex::new(
            r#"(?is)<file>\s*([^<>\r\n]+?\.(?:rs|toml|json))\s*</file>"#
        ).unwrap();
    ...
    for i in 0..count {
        results.push(ParsedFile { path: files[i].clone(), content: codes[i].clone() });
    }
    results
}
...
mod tests {
    ...
    fn test_file_code_pattern() {
        ...
        assert_eq!(parsed.len(), 3, "Expected three parsed file blocks for file/code pattern");
        assert_eq!(parsed[0].path, "Cargo.toml");
        assert!(parsed[0].content.contains("[package]"));
    }
}
